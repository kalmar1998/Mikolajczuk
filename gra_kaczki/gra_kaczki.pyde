...
class Kaczka(Obiekt):
    def obrazek(self):
        ...
        image(loadImage("kaczka2.png"), self.posX, self.posY, 70, 70) # ew. możnaby na początku programu sprawdzić, czy grafiki się wczytują i jeśli nie, nie uruchamiać programu
...
def draw():
    ...
    if bool == True:
        ...
        text(punkty, width/2, height - 50)
        fill(random(255), random(255), random(255)) # jeżli zależy Ci na dynamice, to samo to możńaby wykonywać częściej
        czas = 100
        bool=False   
        
    else:
        czas = czas - 2 # trochę mało tego czasu, trzeba wyważyć, żeby gracz się nie zniechęcał, tylko miał szansę; a jednocześnie było chellenging
...
def mousePressed():
    ...
    if punkty >= 20: # lepiej się zabezpieczyć
        exit() # a nagroda? Jakaś grafika chociaż może?
        
def keyPressed():
    if key == '0':
        exit()
        
 #fajnie, fajnie, ale jako co mam cenić? Brakuje zadania 10 i 11, a projekt to też umiejętność współpracy, dopasowania się i czytania czyjegoś kodu
 #wstawię plusy do aktywności
